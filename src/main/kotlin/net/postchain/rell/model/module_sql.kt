...
abstract class R_ClassSqlMapping {
    ...
    companion object {
        fun makeTransactionBlockHeightExpr(txCls: R_Class, txExpr: Db_TableExpr, chain: R_ExternalChainRef): Db_Expr {
        ...
        fun makeBlockHeightExpr(blockCls: R_Class, blockExpr: Db_TableExpr, chain: R_ExternalChainRef): Db_Expr {
...
class R_ClassSqlMapping_Regular(private val mountName: R_MountName): R_ClassSqlMapping() {
...
class R_ClassSqlMapping_External(private val mountName: R_MountName, private val chain: R_ExternalChainRef): R_ClassSqlMapping() {
    override fun rowidColumn() = SqlConstants.ROWID_COLUMN
...
abstract class R_ClassSqlMapping_TxBlk(
    ...
    abstract fun extraWhereExpr0(cls: R_Class, clsExpr: Db_ClassExpr, chain: R_ExternalChainRef): Db_Expr
...
class R_ClassSqlMapping_Transaction(chain: R_ExternalChainRef?): R_ClassSqlMapping_TxBlk(SqlConstants.TRANSACTIONS_TABLE, "tx_iid", chain) {
    override fun extraWhereExpr0(cls: R_Class, clsExpr: Db_ClassExpr, chain: R_ExternalChainRef): Db_Expr {
        return makeTransactionBlockHeightExpr(cls, clsExpr, chain)
    }
}

class R_ClassSqlMapping_Block(chain: R_ExternalChainRef?): R_ClassSqlMapping_TxBlk(SqlConstants.BLOCKS_TABLE, "block_iid", chain) {
    override fun extraWhereExpr0(cls: R_Class, clsExpr: Db_ClassExpr, chain: R_ExternalChainRef): Db_Expr {
